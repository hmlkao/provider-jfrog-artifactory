...
type CondaRepositoryInitParameters struct {
	...
	// (Boolean) Enable token (Bearer) based authentication. When set, use an access token as password (username may be empty) so Artifactory authenticates to the remote with a Bearer token instead of Basic auth. Default is false for most package types; OCI, Helm OCI, and Hugging Face remotes default to true.
...
// CondaRepository is the Schema for the CondaRepositorys API. Provides a resource to creates a Conda repository.
